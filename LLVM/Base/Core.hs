...
llvmWithContext f = do 
  c <- FFI.llvmCreateContext
  r <- evalStateT f c
...
llvmWithGlobalContext f = do 
  c <- FFI.llvmGetGlobalContext
  evalStateT f c
...
llvmWithModule n f = do 
    m <- liftIO $ llvmCreateModuleWithName n
    evalStateT f m
